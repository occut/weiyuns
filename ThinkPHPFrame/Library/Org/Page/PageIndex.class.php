...
class PageIndex{
    ...
    public function show() {
        ...
        $up_page = $up_row > 0 ? '<a href="'.$this->url($up_row).'">'.$this->config['prev'].'</a>': '<span>'.$this->config['prev'].'</span>';

        //下一页
        $down_row  = $this->nowPage + 1;
        $down_page = ($down_row <= $this->totalPages) ? '<a href="'.$this->url($down_row).'">'.$this->config['next'].'</a>' : '<span>'.$this->config['next'].'</span>';
        ...
        for($i = 1; $i <= $this->rollPage; $i++){
            ...
            if($page > 0 && $page != $this->nowPage){
                ...
                if($page > 0){
                    $link_page .= '<span class="on">'.$page.'</span>';
                }
            }
        }
